...
.fireflies {
  position: fixed;
  inset: 0;
  /* In front of the page, not behind it. Behind the card they would sit
     inside the frosted panel's backdrop, and 15 elements animating forever
     means that blur can never be cached — which showed up as the card
     glitching while the length slider was dragged. pointer-events stays
     none, so nothing here can be clicked. */
  z-index: 3;
  overflow: hidden;
  pointer-events: none;
  opacity: 0;
